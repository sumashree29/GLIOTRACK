...
Fix OLD_SCANS — fallback query reconstruction for scans without query_used stored.
"""
import dataclasses
import logging
...
def _reconstruct_query_for_old_scan(scan_id: str, a1, a2, a3) -> str:
    """
    Reconstruct a fallback query from Agent 2 and Agent 3 outputs for scans
    that don't have query_used stored in agent4_outputs (old scans).
    This ensures clinical context is regenerated for historical scans.
    """
    try:
        from app.agents.clinical_rag_agent import _formulate_query
        
        if a2 and a3 and a1:
            fallback_query = _formulate_query(a2, a3, a1)
            logger.info(
                "Reconstructed query for old scan %s: %s",
                scan_id, fallback_query[:80]
            )
            return fallback_query
    except Exception as exc:
        logger.warning("Query reconstruction failed for scan %s: %s", scan_id, exc)
    
    # Ultimate fallback: generic query based on RANO status
    if a2 and hasattr(a2, 'rano_class') and a2.rano_class:
        return f"Brain tumour response assessment {a2.rano_class} RANO classification neuro-oncology"
    
    return "Brain tumour response assessment RANO classification clinical guidelines"
...
def get_full_report(scan_id: str, request: Request):
    ...
    # FIX OLD_SCANS: Reconstruct query for scans without query_used stored
    if a4_meta and a4_meta.get("query_used"):
        agent4 = _live_rag_query(a4_meta["query_used"])
    elif a4_meta:
        # Old scan without query_used — reconstruct it
        ...
        try:
            if agent4.get("rag_available"):
                from app.agents.clinical_rag_agent import Agent4Output
                a4_updated = Agent4Output(
                    rag_available=agent4["rag_available"],
                    passages=agent4.get("passages", []),
                    failure_reason=agent4.get("failure_reason"),
                    query_used=reconstructed_query,
                )
                upsert_agent4_output(scan_id, a4_updated)
                logger.info("Persisted reconstructed query for scan %s", scan_id)
        except Exception as exc:
            logger.warning("Failed to persist reconstructed query for scan %s: %s", scan_id, exc)
    else:
        agent4 = {
            "rag_available":  False,
            "passages":       [],
